...
import { Button, Card, Col, Form, Image, Input, List, Modal, Row, Select, Statistic, Table, Tabs, Tag, Upload, message } from 'antd';
...
const AdminSystem: React.FC = () => {
  ...
  const loadRules = async () => {
    if (!ensureToken()) return;
    const res = await axios.get(apiUrl('/api/admin/rules'), { headers: adminHeaders });
    setRules(res.data || []);
  };
  ...
  const saveRule = async () => {
    ...
    await axios.post(apiUrl('/api/admin/rules'), values, { headers: adminHeaders });
    setRuleModalOpen(false);
    await loadRules();
  };

  const setRuleCurrent = async (record: any) => {
    if (!ensureToken()) return;
    await axios.post(apiUrl(`/api/admin/rules/${record.id}/set-current`), {}, { headers: adminHeaders });
    await loadRules();
  };
  ...
  const deleteRule = async (record: any) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        await axios.delete(apiUrl(`/api/admin/rules/${record.id}`), { headers: adminHeaders });
        await loadRules();
      }
    });
  };
  ...
  return (
    <div>
      ...
      <Row gutter={12} style={{ marginBottom: 16 }}>
        ...
        <Col xs={24} md={6}>
          <Button type="primary" block onClick={() => confirmRestart()}>
            {t('admin.restartAll')}
          </Button>
        </Col>
      </Row>
      ...
      <Tabs
        items={[
          ...
          },
          {
            ...
            label: t('admin.rules'),
            children: (
              <Card extra={<Button onClick={() => openRuleModal()}>{t('admin.createRule')}</Button>}>
                <Table
                  rowKey="id"
                  dataSource={rules}
                  pagination={{ pageSize: 8 }}
                  columns={[
                    { title: t('admin.ruleName'), dataIndex: 'name' },
                    { title: t('admin.studio'), dataIndex: 'studio' },
                    { title: t('admin.branch'), dataIndex: 'branch' },
                    { title: t('admin.current'), dataIndex: 'is_current', render: (v) => v ? <Tag color="green">Y</Tag> : <Tag>N</Tag> },
                    {
                      title: t('admin.action'),
                      render: (_: any, record: any) => (
                        <div style={{ display: 'flex', gap: 8 }}>
                          <Button size="small" onClick={() => openRuleModal(record)}>{t('admin.edit')}</Button>
                          <Button size="small" onClick={() => setRuleCurrent(record)}>{t('admin.setCurrent')}</Button>
                          <Button size="small" danger onClick={() => deleteRule(record)}>{t('admin.delete')}</Button>
                        </div>
                      )
                    }
                  ]}
                ...
                >
                  <Form form={ruleForm} layout="vertical">
                    <Form.Item name="name" label={t('admin.ruleName')} rules={[{ required: true }]}>
                      <Input />
                    </Form.Item>
                    <Row gutter={12}>
                      <Col span={12}>
                        <Form.Item name="studio" label={t('admin.studio')}>
                          <Input />
                        </Form.Item>
                      </Col>
                      <Col span={12}>
                        <Form.Item name="branch" label={t('admin.branch')}>
                          <Input />
                        </Form.Item>
                      </Col>
                    </Row>
                    <Form.Item name="target_url" label={t('admin.targetUrl')}>
                      <Input />
                    </Form.Item>
                    <Form.Item name="field_mapping" label={t('admin.fieldMapping')}>
                      <Input.TextArea rows={3} />
                    </Form.Item>
                    <Form.Item name="update_frequency" label={t('admin.updateFrequency')}>
                      <Input />
                    </Form.Item>
                    <Form.Item name="exception_policy" label={t('admin.exceptionPolicy')}>
